...
def check_ssl_certificates_dir_setting(ssl_certificates_dir):
    ...
    """
    accepted_values_msg = " Accepted values are either a boolean, in which case it controls whether requests verify" \
    ...
    if isinstance(ssl_certificates_dir, bool):
        if ssl_certificates_dir:
            logger.info("SSL_CERTIFICATES_DIR: SSL certificate verification is enabled.")
        else:
            logger.warning("SSL_CERTIFICATES_DIR: SSL certificate verification is disabled.")

    # if setting is a string
    elif isinstance(ssl_certificates_dir, str):
        # if the path does not exist
        if not exists(ssl_certificates_dir):
            logger.error("SSL_CERTIFICATES_DIR is set but the file or directory does not exist. "
            ...
            if isdir(ssl_certificates_dir):
                ...
                if not listdir(ssl_certificates_dir):
                    logger.error("SSL_CERTIFICATES_DIR is set to an empty directory. {0}".format(accepted_values_msg))
                    raise SSLError("SSL_CERTIFICATES_DIR improperly configured.")
